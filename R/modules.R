...
parse_modules=function(cormat,ldm,modules_version="",nmods=80,reg=1e-6,mod_size=4,min_mod_cor=0.1,zlim=c(-.6,.6)){
...
gene_cor_map=function(cormat,modules_version,zbreaks=c(-1,seq(-1,1,l=99),1),ser_method="OLD_ward",cor_cols=colorRampPalette(c("blue","white","red"))(100)){
  open_plot(fn=paste(modules_version,"gene_cor",sep="_"),plot_type = "pdf",width=ncol(cormat)/12,height=ncol(cormat)/12)
...
sparse.cor <- function(x){
  ...
  sdvec <- as.vector(sqrt(Matrix::diag(covmat))) # standard deviations of columns
  as.matrix(covmat/crossprod(t(sdvec))) # correlation matrix
}
...
cor_analysis=function(ds,ldm){
  
  m1=rowMeans(ds,na.rm=T)
  v1=rowVars(as.matrix(ds),na.rm=T)
  s1=rowSums(ds,na.rm=T) 
  mask1=s1>=50&(!rownames(ds)%in%c(grep("RP",rownames(ds),val=T),grep("MT-",rownames(ds),val=T)))
  x=log10(m1)
  breaks=seq(min(x[mask1]),max(x[mask1]),.2)
  lv=log2(v1[mask1]/m1[mask1])
  llv=split(lv,cut(x[mask1],breaks))
  mask_llv=sapply(llv,length)>0   
  z=sapply(llv[mask_llv],min,na.rm=T)
  b=breaks[-length(breaks)]
  b=b[mask_llv]
  lo=loess(z~b)
  min_varmean_per_gene=0.15
  
  high_var_genes=names(which((lv-predict(lo,newdata =x[mask1]))>min_varmean_per_gene))
...
get_modules_example=function(ldm,modules_version="modules_example",ds_version="2000"){
  
  excluded_clusters=names(grep("Not Good",ldm$clustAnnots,val=T))
  
  cells=names(ldm$dataset$cell_to_sample)[ldm$dataset$cell_to_sample%in%c(inflamed_samples,uninflamed_samples)&!ldm$dataset$cell_to_cluster%in%excluded_clusters]
  
  cormat=get_gene_cormap(ldm,ds_version="2000",cells=cells)
  parse_modules(cormat,ldm,modules_version = modules_version,nmods=80,reg=1e-6,mod_size=4,min_mod_cor=0.1,zlim=c(-.6,.6))
